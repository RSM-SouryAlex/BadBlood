...
function WhySoWeak 
{
    ...
    Set-ADDefaultDomainPasswordPolicy -Identity $id `
        ...
        -ReversibleEncryptionEnabled $false -Verbose

    $Count = 1..50 | Get-Random
    $RandomUsers = $UserList | Get-Random -Count $Count
    foreach($user in $RandomUsers)
    {
        $RandomPassword = Get-random $BadPasswords
        Write-Host $RandomPassword
        $User | Set-ADAccountPassword -Reset -NewPassword (ConvertTo-SecureString $RandomPassword -AsPlainText -Force) -Verbose
    }	    
...
WhySoWeak -UserList (Get-ADUser -Filter 'Samaccountname -ne "krbtgt"')